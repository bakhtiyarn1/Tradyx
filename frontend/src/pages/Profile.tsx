  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }} className="glow-card p-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="glass p-4 text-center">
              <p className="text-xs text-gray-500 mb-1">Referral Earned</p>
              <p className="text-lg font-bold text-purple-400 font-mono">${dashboard?.totalReferralEarned?.toFixed(2) || '0.00'}</p>
            </div>
            <div className="glass p-4 text-center">
              <p className="text-xs text-gray-500 mb-1">Team Size</p>
              <p className="text-lg font-bold text-amber-400 font-mono">{team?.stats?.totalMembers || 0}</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glow-card p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="glass p-3 flex items-center justify-between rounded-xl">
              ...
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={copyCode}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${copiedCode ? 'bg-[#00ff88]/20 text-[#00ff88]' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}>
          ...
          <div className="grid grid-cols-3 gap-3 pt-2">
            <div className="glass p-4 text-center">
              <p className="text-xs text-gray-500 mb-1">L1 Referrals</p>
              <p className="text-2xl font-bold text-[#00ff88]">{team?.stats?.level1Count || 0}</p>
              <p className="text-xs text-gray-600 mt-0.5">10% bonus</p>
            </div>
            <div className="glass p-4 text-center">
              <p className="text-xs text-gray-500 mb-1">L2 Referrals</p>
              <p className="text-2xl font-bold text-[#00d4ff]">{team?.stats?.level2Count || 0}</p>
              <p className="text-xs text-gray-600 mt-0.5">5% bonus</p>
            </div>
            <div className="glass p-4 text-center">
              <p className="text-xs text-gray-500 mb-1">L3 Referrals</p>
              <p className="text-2xl font-bold text-purple-400">{team?.stats?.level3Count || 0}</p>
              <p className="text-xs text-gray-600 mt-0.5">2% bonus</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      {team && (team.level1.length > 0 || team.level2.length > 0 || team.level3.length > 0) && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="glow-card p-6">
          <div className="flex items-center gap-3 mb-5">
          ...
          {[
            { label: 'Level 1 — Direct', color: 'text-[#00ff88]', bg: 'bg-[#00ff88]/10', members: team.level1 },
            { label: 'Level 2', color: 'text-[#00d4ff]', bg: 'bg-[#00d4ff]/10', members: team.level2 },
            { label: 'Level 3', color: 'text-purple-400', bg: 'bg-purple-500/10', members: team.level3 },
          ].map((lvl, idx) => lvl.members.length > 0 && (
            <div key={idx} className="mb-4 last:mb-0">
              <p className={`text-xs font-medium ${lvl.color} mb-2`}>{lvl.label} ({lvl.members.length})</p>
              <div className="space-y-2">
                {lvl.members.slice(0, 10).map(m => (
                  <div key={m.id} className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] hover:bg-white/[0.05] transition-colors">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-lg ${lvl.bg} flex items-center justify-center`}>
