...
function NotificationsDropdown() {
  ...
  const load = async () => { try { const d=await userApi.getNotifications(); setNotifs(d.notifications||[]); setUnread(d.unreadCount||0); } catch {} };
  const markRead = async (id: string) => { try { await userApi.markNotificationRead(id); setNotifs(p=>p.map(n=>n.id===id?{...n,isRead:true}:n)); setUnread(p=>Math.max(0,p-1)); } catch {} };
  const fmt = (d: string) => { const diff=Date.now()-new Date(d).getTime(), m=Math.floor(diff/60000), h=Math.floor(m/60); return h>0?`${h}h`:m>0?`${m}m`:'Now'; };
      ...
      <AnimatePresence>
        {open && <>
          ...
          <motion.div initial={{opacity:0,y:10,scale:0.95}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:10,scale:0.95}} className="absolute right-0 mt-3 w-80 glass p-0 overflow-hidden z-50">
            <div className="p-4 border-b border-white/5 flex items-center justify-between"><h3 className="font-semibold text-white">Notifications</h3>{unread>0&&<button onClick={async()=>{try{await userApi.markAllNotificationsRead();setNotifs(p=>p.map(n=>({...n,isRead:true})));setUnread(0);}catch{}}} className="text-xs text-primary-400 hover:text-[#00ff88] flex items-center gap-1"><Check className="w-3 h-3"/>Mark all read</button>}</div>
            <div className="max-h-80 overflow-y-auto">
              {notifs.length===0?<div className="p-8 text-center"><Bell className="w-8 h-8 text-gray-600 mx-auto mb-2"/><p className="text-gray-500 text-sm">No notifications</p></div>
              :notifs.slice(0,10).map(n=>(
                <motion.div key={n.id} initial={{opacity:0,x:-10}} animate={{opacity:1,x:0}} onClick={()=>!n.isRead&&markRead(n.id)} className={`p-4 border-b border-white/5 hover:bg-white/5 cursor-pointer ${!n.isRead?'bg-primary-500/5':''}`}>
                  <div className="flex items-start gap-3">{!n.isRead&&<span className="w-2 h-2 bg-[#00ff88] rounded-full mt-2 flex-shrink-0 animate-pulse"/>}<div className={!n.isRead?'':'ml-5'}><p className="text-sm text-gray-300">{n.message}</p><p className="text-xs text-gray-500 mt-1">{fmt(n.createdAt)}</p></div></div>
                </motion.div>
              ))}
            </div>
          </motion.div>
...
function UserDropdown() {
  ...
  return (
    <div className="relative">
      ...
      <AnimatePresence>
        {open&&<><motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-40" onClick={()=>setOpen(false)}/>
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div className="lg:pl-72 relative z-10">
        <header className="hidden lg:block sticky top-0 z-30 glass border-b border-white/5">
          <div className="flex items-center justify-between px-8 py-4">
            <div className="live-feed-badge"><span className="dot"/><span>System Online &mdash; Live Feed</span></div>
